      ...
      <View style={[styles.contentStyle, style]}>
        ...
        {followStatus === false ? (
          <TouchableOpacity
            ...
            onPress={this.followTag.bind(this, data)}>
            <TextView style={{fontSize: 9, fontWeight: '700', color: '#fff'}} text='Follow'>
            </TextView>
          </TouchableOpacity>
        ) : (
          <TouchableOpacity
            style={[styles.followButton, {backgroundColor: '#fdbcbc'}]}
            onPress={this.unFollowTag.bind(this, data)}>
            <Customon name="check-circle" color="#fff" />
          </TouchableOpacity>
        )}
      </View>
    );
