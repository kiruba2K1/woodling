...
export default class ProfilePeopleComponent extends Component {
  ...
  render() {
    const { full_name, username, item, user_id } = this.props;
    return (
      <View>
        <View
          style={{
          ...
          <View style={{ alignItems: 'center', flexDirection: 'row' }}>
            {!this.props.followers ?
              <TouchableOpacity onPress={() => this.props.unFollow(item.id)}>
                <FontAwesome5 name="minus" color="red" size={20} />
              </TouchableOpacity>
              :
              item.follow_status == user_id ?
                <TouchableOpacity onPress={() => this.props.unFollow(item.id)}>
                  <FontAwesome5 name="minus" color="red" size={20} />
                </TouchableOpacity>
                :
                <TouchableOpacity onPress={() => this.props.Follow(item.id)}>
